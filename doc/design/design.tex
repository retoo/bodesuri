...
FIXME: Verschoben in Logische Sicht --> Schichtenarchitektur
...
Die Applikationsschicht wurde eingeführt, weil es wegen der Netzwerkfähigkeit viel Synchronisationsarbeit (z.\,B. für Zustände) gibt. Sie hat folgende Hauptaufgaben:
	\begin{itemize}
		...
		\item Controller-Komponente des MVC-Konzeptes, welche die Ereignisse im UI abfängt und verarbeitet, sowie Änderungen an der Problem Domain an das UI weiterleitet.
	\end{itemize}
...
Beim starten des Spieles wird der Spieler aufgefordert die Serverdaten zur Verbindung eizugeben, sowie auch seinen Spielernamen.
...
\begin{figure}
	\centering
	\includegraphics[width=0.7 \textwidth]{gui_lobby}
	\caption{Lobby vor dem Spielbeginn}
	\label{fig:gui_lobby}
\end{figure}

In der Lobby können die Spieler eine Partner für das Spiel auswählen. Der Partner wird ausgewählt und einen Gruppennamen eingegeben. Nun muss nur noch der ausgewählte Partner die Anfrage akzeptieren, um die Gruppe zu bilden. Sobald der Spieler bereit ist, um das Spiel beizutreten wählt er im Spielstatus die CheckBox bereit aus. In der Zwischenzeit können sich die Spieler im Chat unterhalten (Priorität 3).
...
\begin{figure}
	\centering
	\includegraphics[width=0.7 \textwidth]{gui_spiel}
	\caption{Spielbrett}
	\label{fig:gui_spiel}
\end{figure}

Das Spiel besteht aus verschiedenen Views. Im BrettView wird das Spielbrett mit den Feldern und Spielfiguren dargestellt. Im SpielerView werden die einzelnen Spieler mit deren Gruppen aufgelistet. Das KartenView beinhaltet die Karten des Spielers. Im ChatView können sich die Spieler unterhalten (Priorität 3).
...
Aus diesen Gründen haben wir uns dazu entschlossen, für das GUI als einzige Technologie Swing einzusetzen. \textbf{FIXME: Wieviele Stunden haben wir damit verloren?}
