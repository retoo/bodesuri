...
\clearpage
\nomenclature{$Veroderung$}{Das logische Oder von Dingen. In unserem Fall Regeln.}
\nomenclature{$Spielers$}{Wurde im Quellcode verwendet um klar auszudrücken, dass es sich um mehrere Spieler (z.B. ein Array) handelt.}
\nomenclature{$Konkrete Karte$}{Die Karte, für die ein Joker steht.}
\nomenclature{$Aktiver Zustand$}{Ein Zustand der auf einen Event wartet um in den nächsten Zustand über zu gehen.}
\nomenclature{$Passiver Zustand$}{Ein Zustand der direkt in den nächsten Zustand über geht. Quasi ein Epsilon-Übergang.}
\printglossary
