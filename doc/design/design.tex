...
% TODO: Robin: KD ProblemDomain

\clearpage
\subsubsection{Package pd.spiel}

\subparagraph{Beschreibung}
Das Package pd.spiel beinhaltet die beiden Unterpackages pd.spiel.brett und pd.spiel.spieler und die Spiel-Klasse.
...
\clearpage
\subsubsection{Package pd.spiel.brett}

\subparagraph{Beschreibung}
Umfasst das Brett und alle Feldtypen.
...
\clearpage
\subsubsection{Package pd.spiel.spieler}

\subparagraph{Beschreibung}
Beinhaltet den Spieler und was dazugehört, also die Figuren, die Partnerschaft und die SpielerFarbe.
...
\subparagraph{Beschreibung}
Das Regelsystem nimmt die Aufgaben der Problem-Domain-Schicht wahr, indem sie die Validierung der Züge sicherstellt und die daraus resultierenden Aktionen in der Problem-Domain berechnet (Figur von Feld X auf Feld Y verschieben).
...
\clearpage
\subsubsection{Package pd.regelsystem.karten}
...
\subsubsection{Package applikation.client}
	...
	\subsubsection{Package applikation.client.zugautomat, applikation.client.zugautomat.pd, applikation.client.zugautomat.zustaende}
		\subparagraph{Beschreibung}
		Ein eigener Automat inklusive einer kleinen clientspezifischen PD. Wird jede Runde für die Erfassung eines Zuges vom ClientAutomat gestartet und gibt dem Automaten einen fertig erfassten Zug zurück.
