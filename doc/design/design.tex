...
\begin{itemize}
	...
	\item Wenn eine BeitrittsInformation eintrifft wird werden die Informationen über beigetretenen Spieler aktualisiert. Der Zustand wird nicht gewechselt.
...
\subsubsection{AmZug}
\label{ssub:amzug}
Zustand in welchem der Spieler am Zug ist. Erstellt einen ZugAutomat der sich um das Erfassen und Validieren eines Zuges kümmert. Eintreffende Events werden direkt an den ZugAutomaten weitergeleitet. Dieser sendet eine ZugErfasstEvent wenn er fertig ist. Der Event wird versandt und der Automat geht nach NichtAmZug über.
...
\begin{itemize}
	\item Trifft ein FeldGewaehltEvent ein wird das Feld überprüft. Macht die Wahl Sinn wird nach ZielWaehlen gewechselt. Sonst bleiben wir in diesem Zustand.
...
\begin{itemize}
	\item Trifft ein FeldGewaehltEvent ein wird das Feld überprüft. Macht die Wahl Sinn wird nach ZugValidieren gewechselt. Wurde das Startfeld ausgewählt wird das Startfeld wieder deaktiviert und nach StartWaehlen gewechselt.
	\item Trifft ein KarteGewaehltEvent ein (der Spieler hat eine andere Karte ausgewählt) wird nach KarteWaehlen gewechselt.
	\item Trifft ein FeldAbgewaehltEvent ein, wird das Brett zurückgesetzt. Wir bleiben aber in diesem Zustand.
	\item HoverStartEvent und HoverEndeEvent aktiveren bzw. deaktiviern das Markieren des Weges
\end{itemize}
