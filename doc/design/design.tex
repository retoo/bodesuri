...
Das untere Bild stellt den Aufbau des GUIs dar. Das GUI besteht aus mehreren verschachtelten JPanels, welche in der Grafik gelb umrandet sind. Die blau umrandeten Komponenten sind JLabels, welche für Texte wie auch für Bilder verwendet wurden. Die einzelnen Grafiken werden mittels Koordinaten platziert, die in einem XML-Dokument erfasst sind. Beim Erstellen des GUIs werden diese Koordinaten anhand der Klasse BrettXML ausgelesen und entsprechend die Grafiken positioniert.
...
\subsection{Package applikation.client}
	...
	\subsection{Package applikation.client.zugautomat, applikation.client.zugautomat.pd, applikation.client.zugautomat.zustaende}
...
\subsection{Package applikation.server}
	...
	\subsection{Package applikation.server.pd}
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=\textwidth]{kd_server_pd}
			\caption{Klassendiagramm clientspezifische PD}
...
% TODO: Philippe: Vielleicht noch über das Risiko "Fehlerhafte Zeitplanung"  schreiben, von wegen "Reserven einplanen" und so. Dann ginge diese Sektion über eineinhalb Seiten anstatt über eine und eine verquetschte :).
